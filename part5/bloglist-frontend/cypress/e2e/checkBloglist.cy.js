describe('Blog app', function() {
  ...
  describe('When logged in and has 3 blog', function() {
    beforeEach(function() {
      cy.login({ username: 'mluukkai', password: 'salainen' })
      cy.createBlog({ title: 'title of blog', author: 'mluukkai', url: 'http://abc.com', likes: 15 })
      cy.createBlog({ title: 'title of blogh 1', author: 'mluukkai1', url: 'http://abc1.com' })
      cy.createBlog({ title: 'title of blogg 2', author: 'mluukkai2', url: 'http://abc2.com', likes: 10 })
      // cy.contains('title of blog 1')
      // cy.contains('view').click()
      cy.contains('title of blogh 1 mluukkai1').parent().find('button').click() 
      cy.contains('title of blog mluukkai').parent().find('button').click() 
    })

    it('Users can like a blog', function() {
      cy.contains('title of blogh 1').parent().find('button').then( buttons => {
    ...
    it('Blogs are ordered according to likes', function() {
      cy.get('li').eq(0).should('contain', 'title of blogh 1')
      cy.get('li').eq(1).should('contain', 'title of blogg 2')
      cy.get('li').eq(2).should('contain', 'title of blog')
    })
